...
public:
    ...
    inline size_t infer_query_terms(const float *queries_data, const globalIdxType q_start) const
    {
        for (int term = M - 1; term >= 0; --term)
        {
            const float *row = queries_data + q_start + static_cast<size_t>(term) * K;
            for (size_t dim = 0; dim < K; ++dim)
            {
                if (row[dim] != 0.0f)
                {
                    return static_cast<size_t>(term) + 1;
                }
            }
        }
        return 0;
    ...
    DocumentScorer(const string doclens_path, const string decomposed_index_path, const size_t max_query_terms)
    {
        ...
        init_bitvectors(this->n_centroids, this->bitvector_words, this->bitvectors);
        ...
        auto max_doclen = *std::max_element(all_doclens, all_doclens + n_docs);
        buffer_centroids = new int[max_doclen];
    ...
    vector<float> compute_query_centroids_distances(const float *queries_data, const globalIdxType q_start, const size_t query_terms)
    {
        vector<float> current_scores(query_terms * n_centroids);
        int N = n_centroids;
        cblas_sgemm(
            CblasRowMajor,
            CblasNoTrans,
            CblasTrans,
            static_cast<MKL_INT>(query_terms),
            N,
            static_cast<MKL_INT>(K),
            alpha,
            queries_data + q_start,
            static_cast<MKL_INT>(K),
            centroids,
            static_cast<MKL_INT>(K),
            beta,
    ...
    vector<numDocsType>
    find_candidate_docs(const float *queries_data, const globalIdxType q_start, const size_t query_terms, const size_t nprobe, const float th)
    {
        centroids_scores = compute_query_centroids_distances(queries_data, q_start, query_terms);

        vector<size_t> closest_centroids_ids;
        closest_centroids_ids.reserve(nprobe * query_terms);

        for (size_t i = 0; i < query_terms; i++)
        {
            ...
            assign_bitvectors(start_sorted, sorted_indexes - start_sorted, i, this->bitvector_words, this->bitvectors);
    ...
    vector<numDocsType> compute_hit_frequency(vector<numDocsType> &candidate_documents, const size_t k_centroids)
    {
        ...
        for (size_t doc_idx = 0; doc_idx < candidate_documents.size(); doc_idx++)
        {
            ...
            vector<uint64_t> mask(bitvector_words, uint64_t{0});
            for (int i = 0; i < doclen; i++)
            {
                centroid_ids[i] = centroids_assignments[doc_offset + i];
                auto cid = centroids_assignments[doc_offset + i];
                auto bitvector_offset = cid * bitvector_words;
                for (size_t word_idx = 0; word_idx < bitvector_words; ++word_idx)
                {
                    mask[word_idx] |= bitvectors[bitvector_offset + word_idx];
                }
            }
        ...
        reset_bitvectors(this->bitvectors);
    ...
    inline vector<float> compute_ip_with_centroids(const float *queries, const numDocsType doc_id, const size_t query_terms)
    {
        ...
        vector<float> centroid_distances(query_terms * doclen);
        for (int j = 0; j < doclen; j++)
        {
            ...
            {
                // writing transposed
                centroid_distances[j * query_terms + i] = centroids_scores_transposed[centroid_id * M + i];
            }
        }
    ...
    void transpose_centroids_scores_mkl_oplace(const size_t query_terms)
    {
        float alpha = 1.0;
    ...
    inline float compute_score_by_column_reduction_generic(
        const vector<float> &centroid_distances,
        const size_t doclen,
        const size_t M)
    {
        vector<float> max_by_column(M);
        std::copy_n(centroid_distances.begin(), M, max_by_column.begin());

        for (size_t row = 1; row < doclen; ++row)
        {
            const float *row_values = centroid_distances.data() + row * M;
            for (size_t col = 0; col < M; ++col)
            {
                if (row_values[col] > max_by_column[col])
                {
                    max_by_column[col] = row_values[col];
                }
            }
        }

        return accumulate(max_by_column.begin(), max_by_column.end(), 0.0f);
    }

    inline float compute_score_by_column_reduction(const vector<float> &centroid_distances, const size_t doclen, const size_t M)
    {
        return compute_score_by_column_reduction_generic(centroid_distances, doclen, M);
    }
    ...
    vector<numDocsType> second_stage_filtering(const float *queries_data, const globalIdxType q_start, const size_t query_terms, const vector<numDocsType> &doc_ids, const size_t n_documents)
    {
        transpose_centroids_scores_mkl_oplace(query_terms);
        ...
        for (size_t doc_idx = 0; doc_idx < doc_ids.size(); doc_idx++)
        {
            ...
            auto centroid_distances = compute_ip_with_centroids(queries_data + q_start, doc_id, query_terms);

            auto score = compute_score_by_column_reduction_optimal(centroid_distances, doclen, query_terms);
    ...
    {
        __m512i ids = _mm512_loadu_si512((const void *)GLOBAL_INDEXES);
    ...
    inline int *filter_centroids_in_scoring_scalar(const float th, const float *current_centroid_scores, const size_t doclen)
    {
        ...
        for (size_t j = 0; j < doclen; j++)
        {
            ...
            {
                *current_buffer = static_cast<int>(j);
                current_buffer++;
            }
        }
    ...
    vector<tuple<size_t, float>> compute_topk_documents_selected(const float *queries_data, const globalIdxType q_start, const size_t query_terms, const vector<numDocsType> &doc_ids, const size_t k, const float th)
    {
        auto heap = HeapFloats(k);
        pq.precompute_distance_table(queries_data + q_start, query_terms);
        ...
        for (numDocsType doc_id : doc_ids)
        {
            ...
            vector<float> distances(query_terms * doclen);
            vector<float> maxs(query_terms);

            for (size_t i = 0; i < query_terms; i++)
            {
                for (int j = 0; j < doclen; j++)
                {
                    distances[i * doclen + j] = centroid_distances[j * query_terms + i];
                }
