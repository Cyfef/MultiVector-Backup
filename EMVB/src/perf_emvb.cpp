...
int main(int argc, char **argv)
{
    ...
    size_t values_per_query = vec_per_query * len;
    ...
    for (size_t query_id = 0; query_id < n_queries; query_id++)
    {
        ...
        size_t query_terms = document_scorer.infer_query_terms(loaded_query_data, q_start);
        ...
        auto candidate_docs = document_scorer.find_candidate_docs(loaded_query_data, q_start, query_terms, nprobe, thresh);
        ...
        auto selected_docs_2nd = document_scorer.second_stage_filtering(loaded_query_data, q_start, query_terms, selected_docs, out_second_stage);

        // PHASE 4: document scoring
        auto query_res = document_scorer.compute_topk_documents_selected(loaded_query_data, q_start, query_terms, selected_docs_2nd, k, thresh_query);

        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - start).count();
        total_time += elapsed;
        // write top k file

        const size_t result_count = std::min(static_cast<size_t>(k), query_res.size());
