...
namespace multihnsw_query {
    ...
    //     dist_t ans = RunMaxSim(pVec1, pVec2, param) / static_cast<const MultiVectorDoc*>(pVec1) -> doclen;
    //     ans += RunMaxSim(pVec2, pVec1, param) / static_cast<const MultiVectorDoc*>(pVec2) -> doclen;
    ...
    //     assert(static_cast<const MultiVectorDoc*>(pVec1) -> doclen);
    //     return BruteMaxSim(pVec1, pVec2, param) / static_cast<const MultiVectorDoc*>(pVec1) -> doclen;
    // }

    static dist_t
    AveMaxSimDistance(const void* pVec1, const void *pVec2, const void *param) {
        // std::cerr << "index_ave" << std::endl;
    ...
    static dist_t
    AveMaxSimDistance_parallel(const void* pVec1, const void *pVec2, const void *param) {
        // std::cerr << "index_ave" << std::endl;
        const MultiVectorDoc *q = static_cast<const MultiVectorDoc*>(pVec1);
        const MultiVectorDoc *d = static_cast<const MultiVectorDoc*>(pVec2);
        // assert(static_cast<const MultiVectorDoc*>(pVec1) -> doclen && static_cast<const MultiVectorDoc*>(pVec2) -> doclen);
        // std::cerr << "here1" << std::endl;
        dist_t ans = ConstCentroidMaxSim_parallel(q, d) / q -> doclen;
        ans += ConstCentroidMaxSim_parallel(d, q) / d -> doclen;
        ans = 2 - ans;
        // std::cerr << "here2" << std::endl;
        return ans / 2;
    }
    ...
    static dist_t
    UniformedBruteSingleMaxSim(const void *pVec1, const void *pVec2, const void *param) {
        // std::cerr << "index_single" << std::endl;
        const MultiVectorDoc *q = static_cast<const MultiVectorDoc*>(pVec1);
        const MultiVectorDoc *d = static_cast<const MultiVectorDoc*>(pVec2);
        float ret =  1 - BruteMaxSim(q, d, param) / q -> doclen;
    ...
    static dist_t
    UniformedSingleMaxSim_parallel(const void *pVec1, const void *pVec2, const void *param) {
        // std::cerr << "Usingle" << std::endl;
        const MultiVectorDoc *q = static_cast<const MultiVectorDoc*>(pVec1);
        const MultiVectorDoc *d = static_cast<const MultiVectorDoc*>(pVec2);
        float ret =  1 - ConstCentroidMaxSim_parallel(q, d) / q -> doclen;
        // std::cerr << "done." << std::endl;
        return ret;
    }
    ...
    public:
        MultiVectorSpace(hnswlib::SpaceInterface<dist_t>* singleVectorSpace, size_t vdata_len) {
            fstdistfunc_ = AveMaxSimDistance_parallel;
            fstdistfunc_2 = UniformedSingleMaxSim_parallel;
    ...
    class BruteMultiVectorSpace : public hnswlib::AdvancedSpaceInterface<dist_t> {
        hnswlib::DISTFUNC<dist_t> fstdistfunc_;
        hnswlib::DISTFUNC<dist_t> fstdistfunc_2;
        hnswlib::DISTFUNC<dist_t> fstvectordistfunc_;
        // hnswlib::SpaceInterface<dist_t> *singleVectorSpace;
        ...
        // MaxSimParam<dist_t> param;
    public:
        BruteMultiVectorSpace(hnswlib::SpaceInterface<dist_t>* singleVectorSpace, size_t vdata_len) {
            fstdistfunc_ = BruteMaxSim;
            fstdistfunc_2 = UniformedBruteSingleMaxSim;
            fstvectordistfunc_ = singleVectorSpace ->get_dist_func();
            size_t dim = *static_cast<size_t*>(singleVectorSpace -> get_dist_func_param());
            // vdata_len_ = vdata_len;
            data_size_ = _size_doc(vdata_len, dim);
