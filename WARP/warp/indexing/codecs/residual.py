    ...
    def decompress(self, compressed_embs: Embeddings):
        ...
        for codes_, residuals_ in zip(codes.split(1 << 15), residuals.split(1 << 15)):
            if self.use_gpu:
                ...
                # add
                residuals_ = residuals_.to(torch.uint8)
                reversed_bit_map = self.reversed_bit_map.to(torch.uint8)
                decomp_lookup = self.decompression_lookup_table.to(torch.uint8) if self.decompression_lookup_table is not None else None

                # add
                bucket_weights_half = self.bucket_weights.cuda().half() if self.bucket_weights is not None else None
                centroids_half = self.centroids.cuda().half()
