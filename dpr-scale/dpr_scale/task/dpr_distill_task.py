...
from pytorch.text.fb.conf import ModelConf
...
class DPRDistillTask(LightningModule):
    ...
    def _encode_sequence(self, token_ids: List[List[torch.IntTensor]], encoder_model: torch.nn.Module) -> List[List[torch.FloatTensor]]:
        encoded_seq = encoder_model(token_ids)  # bs x d
        return encoded_seq

    def sim_score(self, query_repr:  List[List[torch.IntTensor]], context_repr:  List[List[torch.FloatTensor]], mask=None) ->  List[List[torch.FloatTensor]]:
        scores = torch.matmul(
    ...
    def compute_rank_metrics(self, pred_scores: List[List[torch.FloatTensor]], target_labels: List[torch.IntTensor]) -> List[torch.FloatTensor, torch.FloatTensor]:
    ...
    def encode_queries(self, query_ids: List[List[torch.IntTensor]]) -> List[List[torch.FloatTensor]]:
        #generate qry embeddings
        query_repr = self._encode_sequence(query_ids, self.query_encoder)  # bs x d
        return query_repr

    def forward(self, query_ids: List[List[torch.IntTensor]]) -> List[List[torch.FloatTensor]]:
        # encode qry
    ...
    def _eval_step(self, batch: Dict[str, Any], batch_idx: int) -> Tuple[List[torch.FloatTensor,  torch.FloatTensor], torch.FloatTensor, torch.FloatTensor]:
    ...
    def _eval_epoch_end(self, outputs: Tuple[List[torch.FloatTensor,  torch.FloatTensor], torch.FloatTensor, torch.FloatTensor], log_prefix="valid") -> None:

        total_loss, total_mrr, total_avg_rank, total_count = 0, 0, 0, 0

        for metrics, query_repr, loss in outputs:
            rank, mrr = metrics
            total_avg_rank += rank
            total_mrr += mrr
    ...
    def validation_epoch_end(self, valid_outputs: Tuple[List[torch.FloatTensor,  torch.FloatTensor], torch.FloatTensor, torch.FloatTensor]) -> None:
        self._eval_epoch_end(valid_outputs) if valid_outputs else None

    def test_step(self, batch: Dict[str, Any], batch_idx: int):
        return self._eval_step(batch, batch_idx)

    def test_epoch_end(self, test_outputs: Tuple[List[torch.FloatTensor,  torch.FloatTensor], torch.FloatTensor, torch.FloatTensor]) -> None:
