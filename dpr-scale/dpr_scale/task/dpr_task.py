...
class DenseRetrieverTask(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.in_batch_negatives:
            ...
            query_ctx_mask = torch.ones(query_repr.shape[0], mask.shape[0], dtype=torch.bool)  # bs x ctx_cnt
            for i, pos_ctx_id in enumerate(pos_ctx_indices):
                query_ctx_mask[i, pos_ctx_id:pos_ctx_id+num_ctx_per_batch] = mask[pos_ctx_id:pos_ctx_id+num_ctx_per_batch]
    ...
    def _eval_epoch_end(self, outputs, log_prefix="valid"):
        ...
        if self.in_batch_eval:
            ...
            scores = self.sim_score(all_query_repr, all_context_repr, all_query_ctx_mask)
