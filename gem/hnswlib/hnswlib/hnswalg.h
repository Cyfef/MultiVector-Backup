...
class HierarchicalNSW : public AlgorithmInterface<dist_t> {
 public:
    ...
    std::vector<uint32_t> cluster_label_marks_;
    uint32_t cluster_label_mark_epoch_{1};
    bool cluster_label_marks_enabled_{false};
    ...
    inline bool isNodeAllowedForCluster(tableint internal_id, const std::unordered_set<labeltype> *allowed_labels) const {
        if (cluster_label_marks_enabled_) {
            const labeltype label = getExternalLabel(internal_id);
            return static_cast<size_t>(label) < cluster_label_marks_.size() &&
                   cluster_label_marks_[static_cast<size_t>(label)] == cluster_label_mark_epoch_;
        }
        if (allowed_labels != nullptr) {
            return allowed_labels->find(getExternalLabel(internal_id)) != allowed_labels->end();
        }
        return entry_map.find(internal_id) != entry_map.end();
    }

    void setClusterAllowedLabels(const std::vector<labeltype> &allowed_labels) {
        size_t required_size = max_elements_;
        if (!allowed_labels.empty()) {
            const auto max_label = *std::max_element(allowed_labels.begin(), allowed_labels.end());
            required_size = std::max(required_size, static_cast<size_t>(max_label) + 1);
        }
        if (cluster_label_marks_.size() < required_size) {
            cluster_label_marks_.assign(required_size, 0);
        }
        cluster_label_mark_epoch_++;
        if (cluster_label_mark_epoch_ == 0) {
            std::fill(cluster_label_marks_.begin(), cluster_label_marks_.end(), 0);
            cluster_label_mark_epoch_ = 1;
        }
        for (labeltype label : allowed_labels) {
            if (static_cast<size_t>(label) < cluster_label_marks_.size()) {
                cluster_label_marks_[static_cast<size_t>(label)] = cluster_label_mark_epoch_;
            }
        }
        cluster_label_marks_enabled_ = true;
    }

    void clearClusterAllowedLabels() {
        cluster_label_marks_enabled_ = false;
    }
    ...
    searchBaseLayerClusterAppr(tableint ep_id, const void *data_point, const float *cluster_distance, int layer,
                               const std::unordered_set<labeltype> *allowed_labels = nullptr) {
        tableint currObj = ep_id;
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
        top_candidates = searchBaseLayerClusterConsPara<true>(currObj, data_point, cluster_distance, ef_construction_, allowed_labels);
        while (top_candidates.size() > ef_construction_) {
            top_candidates.pop();
        }
        return top_candidates;
...
//                    if (candidate_id == 0) continue;
    ...
    searchBaseLayerClusterConsPara(
        ...
        while (!candidate_set.empty()) {
            ...
            tableint current_node_id = current_node_pair.second;
            std::vector<tableint> neighbor_ids;
            {
                ...
                for (size_t j = 1; j <= size; ++j) {
                    neighbor_ids.push_back(*(data + j));
                }
            }
//                bool cur_node_deleted = isMarkedDeleted(current_node_id);
            if (collect_metrics) {
                metric_hops++;
                metric_distance_computations += neighbor_ids.size();
            }
            // std::cout << size << " ";
            for (size_t j = 0; j < neighbor_ids.size(); ++j) {
                int candidate_id = neighbor_ids[j];
#ifdef USE_SSE
                if (j + 1 < neighbor_ids.size()) {
                    _mm_prefetch((char *) (visited_array + neighbor_ids[j + 1]), _MM_HINT_T0);
                    _mm_prefetch(data_level0_memory_ + neighbor_ids[j + 1] * size_data_per_element_ + offsetData_,
                                _MM_HINT_T0);  ////////////
                }
#endif
                // if (!(visited_array[candidate_id] == visited_array_tag)) {
                if (!(visited_array[candidate_id] == visited_array_tag) &&
                    isNodeAllowedForCluster((tableint)candidate_id, allowed_labels)) {
                    visited_array[candidate_id] = visited_array_tag;
...
//                    if (candidate_id == 0) continue;
    ...
    size_t getLevel0Degree(tableint p1) {
        std::unique_lock<std::mutex> lock(link_list_locks_[p1]);
        return getListCount(get_linklist0(p1));
    }

    bool canAddEdgeinterWithLimit(tableint p1, size_t max_degree) {
        std::unique_lock<std::mutex> lock(link_list_locks_[p1]);
        return getListCount(get_linklist0(p1)) < max_degree;
    }

    bool hasLevel0Neighbor(tableint p1, tableint p2) {
        std::unique_lock<std::mutex> lock(link_list_locks_[p1]);
        linklistsizeint *ll_other = get_linklist0(p1);
        const size_t sz_link_list_other = getListCount(ll_other);
        tableint *data = (tableint *) (ll_other + 1);
        for (size_t i = 0; i < sz_link_list_other; ++i) {
            if (data[i] == p2) {
                return true;
            }
        }
    ...
    */
    ...
    void addClusterPointEntry(const void *data_point, const float *cluster_distance, labeltype label, labeltype enrty, bool replace_deleted = false) {
        addClusterPointEntry(data_point, cluster_distance, label, enrty, nullptr, replace_deleted);
    }

    void addClusterPointEntry(const void *data_point, const float *cluster_distance, labeltype label, labeltype enrty,
                              const std::unordered_set<labeltype> *allowed_labels, bool replace_deleted = false) {
        if ((allow_replace_deleted_ == false) && (replace_deleted == true)) {
            throw std::runtime_error("Replacement of deleted elements is disabled in constructor");
        }
        ...
        std::unique_lock <std::mutex> lock_label(getLabelOpMutex(label));
        if (!replace_deleted) {
            addClusterPointEntry(data_point, cluster_distance, label, enrty, -1, allowed_labels);
            return;
        }
        // check if there is vacant place
        tableint internal_id_replaced;
    ...
    tableint addClusterPointEntry(const void *data_point, const float *cluster_distance, labeltype label, labeltype entry,
                                  int level, const std::unordered_set<labeltype> *allowed_labels = nullptr) {
        tableint cur_c = 0;
        bool new_point = true;
        volatile int temp = 6;
        {
        ...
        if (label == entry) {
            ...
            std::unique_lock <std::mutex> lock_table(label_lookup_lock);
            auto entry_search = label_lookup_.find(entry);
            if (entry_search == label_lookup_.end()) {
                throw std::runtime_error("Cluster entry label not found");
            }
            currObj = entry_search->second;
        }
        ...
        if (new_point) { 
            ...
            if (label == entry) {
                ...
                int level = 0;
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates = searchBaseLayerClusterAppr(
                        currObj, data_point, cluster_distance, level, allowed_labels);
