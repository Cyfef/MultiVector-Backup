...
extern int NUM_CLUSTER_CALC_RUNTIME;
...
static inline float ClusterHalfBitsToFloatScalar(uint16_t h) {
    int s = (h >> 15) & 0x1;
    int e = (h >> 10) & 0x1F;
    int f = h & 0x3FF;
    if (e == 0) {
        return (s ? -1.0f : 1.0f) * std::ldexp(static_cast<float>(f), -24);
    } else if (e == 31) {
        return (s ? -1.0f : 1.0f) * (f ? NAN : INFINITY);
    }
    return (s ? -1.0f : 1.0f) * std::ldexp(static_cast<float>(f + 1024), e - 25);
}

static inline float ClusterHalfBitsToFloat(uint16_t h) {
    static const std::array<float, 1u << 16> lut = []() {
        std::array<float, 1u << 16> values{};
        for (size_t i = 0; i < values.size(); ++i) {
            values[i] = ClusterHalfBitsToFloatScalar(static_cast<uint16_t>(i));
        }
        return values;
    }();
    return lut[h];
...
static float L2SqrVecClusterEMD(const vectorset* q, const vectorset* p, const float* cluster_dis) {
    ...
    size_t m = p->vecnum ;
    std::vector<double> dist_flat(n * m);
    const uint16_t* cluster_dis_half = reinterpret_cast<const uint16_t*>(cluster_dis);
    // std::cout << n << " " << m << std::endl;

    // #pragma omp parallel for schedule(dynamic)
    for (size_t i = 0; i < n; ++i) {
        long long icode = (long long)q->codes[i] * NUM_CLUSTER_CALC_RUNTIME;
        // std::cout << i << " " << icode << " "  << std::flush;
        for (size_t j = 0; j < m; ++j) {
            dist_flat[i * m + j] = (double)1.0f - ClusterHalfBitsToFloat(cluster_dis_half[icode + p->codes[j]]);
        }
    }
...
static float L2SqrCluster4Search(const vectorset* q, const vectorset* p, int level) {
    ...
    size_t q_vecnum = q->vecnum;
    size_t p_vecnum = p->vecnum;
    sum1 = compute_with_eigen(q->data, p->codes, q->vecnum, NUM_CLUSTER_CALC_RUNTIME, p->vecnum);
    return sum1 / q->vecnum;
}
...
static float L2SqrClusterAVX4Search(const vectorset* q, const vectorset* p, int level) {
    ...
    for (int i = 0; i < q->vecnum; ++i) {
        ...
        for (size_t j = 0; j < p_vecnum; j += 16) {  // 每次处理 16 个列索引
            ...
            // 按索引读取数据
            __m512 values = _mm512_i32gather_ps(indices, &(q->data[i * NUM_CLUSTER_CALC_RUNTIME]), sizeof(float));
