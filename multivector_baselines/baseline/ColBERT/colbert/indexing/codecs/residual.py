...
class ResidualCodec:
    ...
    @classmethod
    def load(cls, index_path, is_build_index, use_gpu=False):
        config = ColBERTConfig.load_from_index(index_path)
        
        # [Modified] Allow forcing GPU usage via argument
        if use_gpu:
            config.total_visible_gpus = 1
        elif not is_build_index:
