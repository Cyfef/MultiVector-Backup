...
class CollectionIndexer():
    ...
    def run(self, embedding_filename, shared_lists):
        with torch.inference_mode():
            ...
            start_time = time.time()
            encode_passage_time = self.index(embedding_filename)  # Encodes and saves all tokens into residuals
            distributed.barrier(self.rank)
    ...
    def _sample_embeddings(self, sampled_pids):
        ...
        local_sample_embs, doclens = self.encoder.encode_passages(local_sample, self.collection.has_embedding)
    ...
    #     # 生成用于计算残差(avg_residual)的 heldout 验证集
    #     sample, heldout = self._concatenate_and_split_sample()

    #     # ====================  ====================
    #     # 不再调用 _train_kmeans，而是直接从文件加载预先计算好的质心

    #     print_message("#> 跳过 K-Means 训练步骤。")
    #     print_message("#> 正在从外部文件加载预先计算的质心(centroids)...")

    #     # --- 请务必将这里的路径修改为您 `centroids.npy` 文件的实际存放路径 ---
    #     centroids_path = "/data1/lijunlin/clerc_centroids.npy"
        
    #     try:
    #         # 使用 numpy 加载 .npy 文件
    #         centroids_np = np.load(centroids_path)
    #     except FileNotFoundError:
    #         print(f"错误：找不到质心文件！请确保路径正确: {centroids_path}")
    #         raise
        
    #     # 将 Numpy 数组转换为 PyTorch 张量
    #     # .to(heldout.device, dtype=heldout.dtype) 这步至关重要，
    #     # 它确保了加载的质心与ColBERT内部处理的张量在同一个设备上（例如GPU）并且数据类型一致。
    #     centroids = torch.from_numpy(centroids_np).to(heldout.device, dtype=heldout.dtype)

    #     print_message(f"#> 成功加载质心，形状为: {centroids.shape}")
        
    #     # 验证加载的质心维度是否与样本向量的维度匹配
    #     assert centroids.shape[1] == heldout.shape[1], \
    #         f"质心维度 ({centroids.shape[1]}) 与向量维度 ({heldout.shape[1]}) 不匹配!"

    #     # 原有的 _train_kmeans 调用已被上面的代码块完全取代。
    #     # centroids = self._train_kmeans(sample, shared_lists)
    #     # ====================  ====================

    #     print_memory_stats(f'RANK:{self.rank}')
    #     # sample 训练集在我们的流程中不再需要，可以提前删除以释放内存
    #     del sample

    #     # 使用您加载的 'centroids' 和上面生成的 'heldout' 验证集，继续ColBERT的原有流程
    ...
    def _compute_avg_residual(self, centroids, heldout):
        ...
        heldout_avg_residual = heldout_avg_residual[torch.randperm(len(heldout_avg_residual))[:1000]]
    ...
    def index(self, embedding_filename):
        ...
        with self.saver.thread():
            if not self.collection.has_embedding:
                batches = self.collection.enumerate_batches(rank=self.rank)
            else:
                batches = self.collection.enumerate_batches2(rank=self.rank)
            for data in tqdm.tqdm(batches, disable=self.rank > 0):
                # if not self.collection.has_embedding:
                chunk_idx, offset, passages = data
                ...
                start_time = time.time()
                embs, doclens = self.encoder.encode_passages(passages, has_embedding=self.collection.has_embedding)
                # else:
                #     chunk_idx, offset, embs = data
                if self.use_gpu:
                    embs = embs.half()

                encode_passage_time += time.time() - start_time
