...
def resolve_base_model_path(name_or_path):
    if name_or_path and os.path.exists(name_or_path):
        return name_or_path

    env_model_path = os.environ.get("COLBERT_BASE_MODEL_PATH")
    if env_model_path and os.path.exists(env_model_path):
        return env_model_path

    candidate_m = {
        "bert-base-uncased": [
            "/data/lijunlin/model/google-bert/bert-base-uncased",
            "/data1/lijunlin/model/AI-ModelScope/bert-base-uncased",
        ],
    }

    for candidate in candidate_m.get(name_or_path, []):
...
def class_factory(name_or_path):
    resolved_name_or_path = resolve_base_model_path(name_or_path)
    loadedConfig  = AutoConfig.from_pretrained(resolved_name_or_path)
