...
class CandidateGeneration:
    ...
    def generate_candidate_pids(self, Q, ncells):
        # cells is the centroid ID, scores the is IP between all query vectors and the centroids
        cells, scores = self.get_cells(Q, ncells)

        pids, cell_lengths = self.ivf.lookup(cells)
        
        # === [新增] 打印聚类统计信息 ===
        # 只有当使用 CPU 或者显式开启 debug 时打印，避免刷屏
        # 这里直接打印以便您观察
        print(f"\n>>> [Cluster Stat] ncells={ncells}")
        print(f"    - Unique Centroids Hit: {len(cells)} (Query去重后命中的聚类中心数)")
        print(f"    - Vectors per Centroid (First 20): {cell_lengths[:20].tolist()} ...")
        print(f"    - Min/Max/Avg Vectors per Centroid: {cell_lengths.min().item()} / {cell_lengths.max().item()} / {cell_lengths.float().mean().item():.1f}")
        print(f"    - Total Vectors (Candidates Pre-unique): {cell_lengths.sum().item()}")
        # ==============================
    ...
    def generate_candidates(self, config, Q):
        ...
        Q = Q.squeeze(0)
        if self.use_gpu:
            print("warning !!! use GPU for searching")
            Q = Q.cuda().half()
        assert Q.dim() == 2
