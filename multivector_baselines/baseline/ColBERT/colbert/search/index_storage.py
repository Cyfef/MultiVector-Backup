...
class IndexScorer(IndexLoader, CandidateGeneration):
    def __init__(self, index_path, use_gpu=True):
        ...
        total_centroids = self.codec.centroids.size(0)
        embedding_dim = self.codec.centroids.size(1)
        print(f"\n>>> [Index Info] Total Centroids: {total_centroids}, Dimension: {embedding_dim}")
