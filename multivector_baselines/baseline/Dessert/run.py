...
def build_index(username: str, dataset: str,
    ...
    centroids_npy = os.path.join(dessert_index_path, 'centroids.npy')
    centroid_ids_npy = os.path.join(dessert_index_path, 'all_centroid_ids.npy')

    if not (os.path.exists(centroids_npy) and os.path.exists(centroid_ids_npy)):
        delete_file_if_exist(dessert_index_path)
        os.makedirs(dessert_index_path, exist_ok=False)

        centroids = torch.load(os.path.join(plaid_index_path, 'centroids.pt'), map_location=torch.device('cuda'))
        # np.save(os.path.join(dessert_index_path, 'centroids.npy'), centroids.numpy().astype('float32'))
        np.save(centroids_npy, centroids.cpu().numpy().astype('float32'))
        ...
        for chunkID in range(n_chunk):
            ...
            total_centroid_id = np.concatenate([total_centroid_id, centroid_id_l.cpu().numpy()])
        ...
        np.save(centroid_ids_npy, total_centroid_id.astype('int32'))
