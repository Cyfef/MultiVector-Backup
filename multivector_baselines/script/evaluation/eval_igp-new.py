...
def approximate_solution_build_index(username: str, dataset: str,
    ...
    if save_index:
        index_path = f'/data1/{username}/Dataset/multi-vector-retrieval/Index/{dataset}/{module_name}-{build_index_suffix}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='gte-small-multi')# 'openai_gte_small'  openai_gpt2_large
    ...
    config_l = {
        ...
        'local': {
            ...
            'build_index_parameter_l': [
                ...
                # --- final test ---
                {'n_centroid_f': lambda x: int(2 ** np.floor(np.log2(8 * np.sqrt(x)))), 'n_bit': 2},
                # {'n_centroid_f': lambda x: int(2 ** np.floor(np.log2(64 * np.sqrt(x)))), 'n_bit': 4},
                {'n_centroid_f': lambda x: int(2 ** np.floor(np.log2(8 * np.sqrt(x)))), 'n_bit': 4},
            ...
            'grid_search_para': {
                ...
                10: {
                    'nprobe': [1, 4, 8, 32],
                    'probe_topk': [10, 100, 1000],
                },
                100: {
                    'nprobe': [1, 8, 16, 32],
                    'probe_topk': [100, 1000, 10000],
                },
            }
        }
    }
