...
def approximate_solution_retrieval(index: object, retrieval_config: dict, query_l: np.ndarray, topk: int):
    ...
    (est_dist_l, est_id_l, retrieval_time_l,
     ...
     n_search_candidate_l, query_mat) = index.search(
        query_l=query_l, topk=topk,
        n_candidate=n_candidate)
    # single_vec_embedding_file_name = os.path.join(f'/data/{username}/Dataset/multi-vector-retrieval/RawData/{dataset}/document/transformed_embeddings/single_vector_query_embedding.npy')
    
    # np.save(single_vec_embedding_file_name, query_mat)
...
def transform_multi_vector_to_single_vector(itemlen_l_chunk,item_vecs_l_chunk, random_matrix_l_cuda, partition_vec_l_cuda, module):
    ...
    print("shape::", vec_cluster_bit_l.shape)
    ...
    print("shape::", cluster_vector_l.shape)
...
def build_index(username: str, dataset: str, index: object, module: object,
    ...
    # Dataset/multi-vector-retrieval/RawData/openai/document/transformed_embeddings/
    single_vec_embedding_file_name = os.path.join(f'/data/{username}/Dataset/multi-vector-retrieval/RawData/{dataset}/document/transformed_embeddings/single_vector_embedding.npy')
    ...
    query_lens = np.load(os.path.join(embedding_dir, 'query_n_vec_length.npy'))
    query_concat_l = np.concatenate([query_l[idx][0:query_lens[idx]] for idx in range(len(query_lens))])
    ...
    data_single_vec = transform_multi_vector_to_single_vector(itemlen_l_chunk,item_vecs_l_chunk, random_matrix_l_cuda, partition_vec_l_cuda, module)
    
    query_single_vec = transform_multi_vector_to_single_vector(query_lens,query_concat_l, random_matrix_l_cuda, partition_vec_l_cuda, module)
    
    print("similarity between query single vector and data single vector:", np.dot(query_single_vec[-1], data_single_vec[-1]))
    
    single_vec_query_embedding_file_name = os.path.join(f'/data/{username}/Dataset/multi-vector-retrieval/RawData/{dataset}/document/transformed_embeddings/single_vector_query_embedding2.npy')
    
    np.save(single_vec_query_embedding_file_name, query_single_vec)
    
    single_vec_embedding_file_name = os.path.join(f'/data/{username}/Dataset/multi-vector-retrieval/RawData/{dataset}/document/transformed_embeddings/transformed_single_vector_embedding/')
    
    os.makedirs(single_vec_embedding_file_name, exist_ok=True)
    print(f"save single vector embedding to {single_vec_embedding_file_name}")
    ...
    for chunkID in tqdm.trange(n_chunk):
        ...
        np.save(os.path.join(single_vec_embedding_file_name, f'single_vector_embedding{chunkID}.npy'), ip_vector_l_chunk)
        itemID_offset += n_item_chunk
        vec_l[n_vec_offset:n_vec_offset + n_vec_chunk] = item_vecs_l_chunk
        n_vec_offset += n_vec_chunk

    
    # np.save(single_vec_embedding_file_name, ip_vector_l)
...
def approximate_solution_build_index(username: str, dataset: str,
    ...
    result_performance_path = f'/data/{username}/Dataset/multi-vector-retrieval/Result/performance'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='openai')
    ...
    config_l = {
        ...
        'local': {
            ...
            'is_debug': True,
            'build_index_parameter_l': [
                {'k_sim': 5, 'd_proj': 16, 'r_reps': 20, 'R': 20, 'L': 50, 'alpha': 1.2, 'n_centroid_per_subspace': 256, 'dim_per_subspace': 16},
                {'k_sim': 5, 'd_proj': 64, 'r_reps': 40, 'R': 20, 'L': 50, 'alpha': 1.2, 'n_centroid_per_subspace': 256, 'dim_per_subspace': 16},
                {'k_sim': 1, 'd_proj': 16, 'r_reps': 2, 'R': 20, 'L': 50, 'alpha': 1.2, 'n_centroid_per_subspace': 256, 'dim_per_subspace': 16},
            ],
            'retrieval_parameter_l': [
                {'n_candidate': 20},
                {'n_candidate': 30},
