...
def compress_into_codes(embs: np.ndarray, centroid_l_cuda: torch.Tensor):
    ...
    for batch in embs.split(bsize):
        indices = (centroid_l_cuda.to('cpu') @ batch.T.to('cpu').float()).max(dim=0).indices
...
def compute_assignment(username: str, dataset: str,
    ...
    centroid_l_cuda = torch.Tensor(centroid_l).to('cpu')
...
def faiss_kmeans(sample_vecs_l: np.ndarray, n_centroid: int):
    ...
    # # 1. 我们不再需要
    # #    - 初始化 faiss.Kmeans 对象
    # #    - 调用 kmeans.train() 进行训练
    # #    因此，下面三行原始代码可以被安全地删除或注释掉。
    ...
    # # 2. 直接从 .npy 文件加载质心
    # print("#> skip K-Means loading cluster...")
    
    # centroids_path = "/data1/lijunlin/clerc_centroids.npy"
    

    # loaded_centroids_np = np.load(centroids_path)

    # print(f" {centroids_path} loaded!")
    # print(loaded_centroids_np.shape)

    # centroids = torch.from_numpy(loaded_centroids_np)
    # # ==================== ====================
...
def faiss_kmeans(sample_vecs_l: np.ndarray, n_centroid: int):
    print("build kmeans")
    vec_dim = sample_vecs_l.shape[1]
    kmeans = faiss.Kmeans(vec_dim, n_centroid, niter=20, gpu=faiss.get_num_gpus() > 0, verbose=True, seed=123)
    kmeans.train(sample_vecs_l)
